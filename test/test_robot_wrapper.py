...
@pytest.mark.skipif(
    ...
    reason="Test requires Stretch robot URDF and assets.",
)
@pytest.mark.skipif(
    not habitat_sim.built_with_bullet,
    reason="Robot wrapper API requires Bullet physics.",
...
def test_stretch_robot_wrapper(fixed_base):
    ...
    with habitat_sim.Simulator(hab_cfg) as sim:
        ...
        # add the robot to the world via the wrapper
        robot_path = "data/robots/hab_stretch/robots/hab_stretch/urdf/hab_stretch_dex_wrist_simplified.urdf"
        stretch = stretch_robot.StretchRobot(robot_path, sim, fixed_base=fixed_base)
        stretch.reconfigure()
        ...
        # arm joint queries and setters
        print(f" Arm joint velocities = {stretch.arm_velocity}")
        stretch.arm_joint_pos = np.ones(len(stretch.params.arm_joints))
        stretch.arm_motor_pos = np.ones(len(stretch.params.arm_joints))
        print(f" Arm joint positions (should be ones) = {stretch.arm_joint_pos}")
        print(f" Arm joint limits = {stretch.arm_joint_limits}")
        stretch.arm_motor_pos = stretch.arm_motor_pos
        observations += simulate(sim, 1.0, produce_debug_video)
        ...
        assert np.allclose(
            stretch.gripper_joint_pos,
            stretch.params.gripper_open_state,
        )
        assert stretch.is_gripper_open
        observations += simulate(sim, 0.2, produce_debug_video)
